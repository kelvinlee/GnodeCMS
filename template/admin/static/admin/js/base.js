...
var CKupdate, freepost, post, removepost;
...
$(document).ready(function() {
  ...
  if ($("[name=dropzone]").length > 0) {
    $("[name=dropzone]").dropzone({
      url: "/admin/media/upload",
      addRemoveLinks: "/admin/media/del"
    });
  }
  if ($('.pics').length > 0) {
    return $('.pics').tagsInput({
      defaultText: "图片名"
    });
  }
});

removepost = function(obj, callback) {
  var yestopost;
  if (callback == null) {
    callback = null;
  }
  ...
  if (confirm("你确定要删除'" + ($(obj).attr("title")) + "'")) {
    yestopost();
  }
  return false;
};

freepost = function(url, data, callback) {
  if (callback == null) {
    callback = null;
  }
  CKupdate();
...
CKupdate = function() {
  ...
  for (instance in CKEDITOR.instances) {
    results.push(CKEDITOR.instances[instance].updateElement());
  }
  return results;
};
