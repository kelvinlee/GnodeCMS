...
exports.index = (req,res,next)->
	ep = new EP.create "urls", (urls)->
		data = {}
		data.urls_list = urls
		res.render config.templateforadmin+"/recommend-index",data

	Urls.getAll (err,urls)->
		ep.emit "urls" ,urls
...
exports.readed = (req,res,next)->
	Path = res.locals.Path
	console.log Path
	if Path[4]?
		Urls.findOne {_id:Path[4]},(err,urls)->
			if err
				console.log err
				next()
				return 
			urls.readed = true
			urls.read_at = new Date()
			urls.save()
			res.send helper.recode()
			Logs.new "Urls",req.session.loginAdmin or req.cookies.admin_user,"Readed #{urls._id} ."
	else
		next()
